...
启用 Web Console 后，管理员可以通过草稿配置 API 编辑 Command Catalog、`security.token`、`security.allow`、`security.http_hosts` 和 `runtime.catalog_notify`。保存时仍会走完整配置校验，校验失败不会写盘，也不会热应用。
