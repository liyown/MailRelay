...
go vet ./...
task dev                      # watch files, rebuild embedded console + Go binary, and restart via Air
```
...
1. `mail.Parse` extracts sender, token, command name, params (`key=value` or JSON object), and the raw body used by `http_request`.
...
- `handler` — one file per handler type plus shared HTTP helpers; all satisfy `command.Handler`. `registry.go` maps names to instances. Builtins registered in `buildRouter`: `http`, `http_request`, `webhook`, `workflow`, `queue` (stable) and `plugin`, `shell`, `agent`, `mcp` (experimental).
...
- `security` — `Authenticate` (allowlist + constant-time token) and `NetworkPolicy` (outbound SSRF guard: rejects private/loopback/link-local/metadata addresses at validate and dial time; keeps URL scheme/credentials/host static for configured HTTP commands; rejects cross-host redirects).
- `web` — embedded console HTTP server, session auth (Argon2id). `assets.go` serves the embedded SPA. Read endpoints deliberately never return mailbox credentials, web session secrets, full mail bodies, or raw provider errors. The console can also **edit** the command catalog, `security.token`, `security.allow`, `security.http_hosts`, and `runtime.catalog_notify` via `GET`/`PUT /api/v1/config/draft` (session + CSRF). Writes go through `Runtime.ApplyDraft`: it validates against the full config (`config.Load` + `Validate`) before persisting, so email can still only trigger validated commands. Persistence is a surgical `yaml.Node` edit (`config.RenderDraft`) of only those editable sections — it never re-serializes the loaded `Config` (env `${VAR}` is resolved before parse, so that would bake secrets into the file); mail credentials, the session secret, `${VAR}` refs, and comments outside edited sections are preserved, then the temp file is atomically renamed and hot-applied. The draft endpoints (behind auth+CSRF) expose editable command config and token values to admins, so keep handler secrets in `${VAR}`/`sensitive` params, not inlined. Invalid drafts return `422 invalid_config` with the (safe, self-authored) validation message and never touch the file.
- `cli` — the `mailrelay` subcommands (`init`, `run`, `once`, `status`, `doctor`, `replay`, `soak`, `hash-password`, `version`).

## Handler contract

Every handler implements `command.Handler` and is resolved only by its configured name — email can never select an arbitrary executable or tool. When adding a handler: register it in `buildRouter` (`internal/app/runtime.go`), keep outbound HTTP going through `security.NetworkPolicy`, and return a `command.Error` with an appropriate `Kind` so `App.classify` maps it to a safe reply. Configured HTTP commands may template path segments and `config.query`, but scheme/credentials/host stay static; `http_request` may forward an absolute or origin-form request body and must still pass outbound policy. `queue` params cannot be `sensitive` because delayed execution persists plaintext; `workflow` step params must explicitly map target command parameters and preserve sensitive-to-sensitive mappings.
...
- Never persist tokens, mailbox passwords, API keys, or full mail bodies. `sensitive: true` params are redacted from audit records via `security.Redact`; HTTP-family handler logs may include truncated request/response transcripts, so always run them through the snapshot redaction helpers.
